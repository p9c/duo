...
func TestBytes(t *testing.T) {
	...
	ap = &ab
	al.buf = ap
	fmt.Println(New().Len(), al.Len(), New().Elem(0), az.Elem(50), al.Elem(1))
	fmt.Println(New().Size(), az.Size(), ay.Size())
	fmt.Println(New().Rand(32), az.Rand(23), ay.Rand(13), az.Rand(-3))
	fmt.Println(az.SetCoding("hex").(*Bytes).String(), ay.Null(), az.Null(), New().Null(), ay.New(32))
	...
	fmt.Println(ay.Copy(az), az.Copy(an), az.Copy(az), az.Copy(New().Rand(32)))
}
