...
func (r *Bytes) Copy(b def.Buffer) def.Buffer {
	switch {
	case nil == r:
		r = nilError("Copy()")
		fallthrough
...
func (r *Bytes) Link(b interface{}) (R def.Buffer) {
	switch b.(type) {
	case *Bytes:
		switch {
		case nil == r:
...
func (r *Bytes) Load(bytes interface{}) (R def.Buffer) {
	if nil == bytes {
		r = nilError("Load()")
	}
	switch bytes.(type) {
...
func (r *Bytes) Move(buf def.Buffer) (R def.Buffer) {
	switch {
	case nil == r:
		r = nilError("Move()")
		fallthrough
...
func (r *Bytes) New(size int) (R def.Buffer) {
	if nil == r {
		r = nilError("New()")
	}
	x := make([]byte, size)
...
func (r *Bytes) Null() (R def.Buffer) {
	if nil == r {
		r = nilError("Null()")
	} else {
		if nil == r.buf {
...
func (r *Bytes) Rand(size ...int) (R def.Buffer) {
	if nil == r {
		r = nilError("Rand()")
	}
	if len(size) > 0 {
...
func (r *Bytes) Coding() string {
	...
	if r.coding > len(def.CodingTypes) || r.coding < 0 {
